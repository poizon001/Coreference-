...
singularPronounsNoMF = ["i", "me", "myself", "mine", "my", "it", "itself", "its", "one", "oneself", "one's"]
		...
		if i > 0 and i < l-1:
			...
			if "NN" in pos:
				if curr in prev or curr in next:
					# print "removing ", list_of_nps[i]
					list_of_nps.remove(list_of_nps[i])
					tmp_lst_pos.remove(tmp_lst_pos[i])
					i -= 1
			# print "len->", l, "ii->",i
		l = len(list_of_nps)
		i += 1
								...
								if len(k2) > 1:
									...
									for n in np:
										if n in last_name_dict:
											print "nnnnn", n
											flag = 1
											idx1 = get_index(lst_np_hw_sentnum_npnum, k2, l-j-1)
											idx2 = get_index(lst_np_hw_sentnum_npnum, w, l)
											print "idx"
											print idx1
											print idx2
	...
	while l >= 0:
		...
		for i in np_lst:
			if i.lower() in pluralPronouns:
				w = i
				idx1 = get_index(lst_np_hw_sentnum_npnum, w, l)
						...
						for k in range(len(np_lst_up)):
							...
							if k < len(np_lst_up):
								...
								while k < len(np_lst_up):
									...
									idx2 = get_index(lst_np_hw_sentnum_npnum, ww, n)
									# print "########",lst_sent_np[n]
...
def corefer_plural_up(lst_of_sent, lst_sent_np, lst_np_rel_sent, start_sent, level_up, last_name):
	...
	for i in reversed(range(n)):
		...
		for j in reversed(lst_sent_np[i]):
			...
			if j.lower() in pluralPronouns:
				...
				if FLAG == 0:
					print "\n going upwards \n"
					corefer_plural_up(lst_of_sent, lst_sent_np, lst_np_rel_sent, i, 3, last_name)
...
def pronoun_frst_person(lst_of_sent, lst_sent_np, lst_np_hw_sentnum_npnum,lst_np_rel_sent):
			...
			if w.lower() in singularPronounsNoMF:
				...
				for idx in range(i):
					...
					if pos == "NNP" and rel == "nsubj":
						flag = 1
						print "w1",w1,"w2",w2
						idx1 = get_index(lst_np_hw_sentnum_npnum, w2, l)
						idx2 = get_index(lst_np_hw_sentnum_npnum, w1, l)
						print "idx"
						print idx1
						print idx2
						for ll in lst_np_hw_sentnum_npnum:
	...
	return get_lst_np_hw_sentnum_npnum
			...
			if w1.lower() in secondPersonPronouns:
				...
				for k in reversed(range(idx)):
					...
					if pos == "NNP":
						...
						idx1 = get_index(lst_np_hw_sentnum_npnum, w1, l)
						idx2 = get_index(lst_np_hw_sentnum_npnum, w2, l)
						print "idx"
						print idx1
						print idx2
						for l in lst_np_hw_sentnum_npnum:
...
if __name__ == "__main__":
	...
	get_lst_np_hw_sentnum_npnum = pronoun_frst_person(lst_of_sent, lst_sent_np, lst_np_hw_sentnum_npnum,lst_np_rel_sent)
	print "\ncluster 4 \n"
	for i,j in enumerate(lst_np_hw_sentnum_npnum):
		print i, j
...
#np_plural_nnp  hard rule for only 2 NNP there can be 3 or more nnp in continuation
#rule for 2 bacause in example "President Obama and Prime Minister Modi are coming to india. They will be visiting to places."
#3rd nno i.e india is also NNP but shold not be coreferred by they