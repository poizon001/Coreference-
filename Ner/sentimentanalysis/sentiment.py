...
def get_np(tree, origtree):
	for subTree_dic in tree:
		...
		if len(inside_trees) == 0:
			...
			get_np(inside_trees, origtree)



# tre=[[{u'node': u'-> said-VBD (root)', u'subTree': [{u'node': u'  -> Global-NNP (nsubj)', u'subTree': [{u'node': u'    -> Tiger-NNP (nn)', u'subTree': []}, {u'node': u'    -> investor-NN (appos)', u'subTree': [{u'node': u'      -> big-JJ (amod)', u'subTree': []}, {u'node': u'      -> in-IN (prep)', u'subTree': [{u'node': u'        -> start-ups-NNS (pobj)', u'subTree': [{u'node': u'          -> tech-NN (nn)', u'subTree': []}]}]}]}]}, {u'node': u'  -> plan-VB (xcomp)', u'subTree': [{u'node': u'    -> to-TO (aux)', u'subTree': []}, {u'node': u'    -> $-$ (dobj)', u'subTree': [{u'node': u'      -> new-JJ (amod)', u'subTree': []}, {u'node': u'      -> 1.5-CD (num)', u'subTree': []}, {u'node': u'      -> fund-NN (dep)', u'subTree': [{u'node': u'        -> Billion-NNP (nn)', u'subTree': []}]}]}]}]}]]
# print 
# tree_parse(tre[0])
# extract_subj_obj(tre[0])