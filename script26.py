...
allPronouns = ["who", "whom", "whose","i", "me", "myself", "mine", "my","we", "us", "ourself", "ourselves", "ours", "our", "they", "them", "themself", "themselves", "theirs", "their","he", "him", "himself", "his","her", "hers", "herself", "she","it", "its", "itself"]
...
def fix_lst_np(list_of_nps, tmp_lst_pos, lst_head_tmp, tmp_lst_rel, tmp_lst_lvl):
	...
	temp_lst = ast.literal_eval(str(list_of_nps))
	temp_hw = []

	if l != len(lst_head_tmp):
		for i in range(len(temp_lst)):
			w = temp_lst[i].split()
			temp_hw.append(w[len(w)-1])

		lst_head_tmp = temp_hw		
	# print l , l1

	print len(list_of_nps)
	print len(lst_head_tmp)
	...
	if l > 1:
		for j,i in enumerate(list_of_nps):
			w = i
			nps = w.split()
			print i, "-> ", nps
			if "their" in nps:
				...
				tmp_np.append("their")
				tmp_pos.append('PRP')
				tmp_head.append("their")
				tmp_lvl.append(tmp_lst_lvl[j])
				tmp_rel.append(tmp_lst_rel[j])

				tmp_np.append(i)
				tmp_pos.append(tmp_lst_pos[j])
				tmp_head.append(lst_head_tmp[j])
				tmp_lvl.append(tmp_lst_lvl[j])
				tmp_rel.append(tmp_lst_rel[j])

	list_of_nps.extend(tmp_np)
	tmp_lst_pos.extend(tmp_pos)
	lst_head_tmp.extend(tmp_head)
	tmp_lst_rel.extend(tmp_rel)
	tmp_lst_lvl.extend(tmp_lvl)
...
def get_lst_sent_np_rel_lvl_head(lst_of_sent):
	for i,j in enumerate(lst_of_sent):
		...
		lst_head_tmp = fix_lst_np(tmp_lst, tmp_lst_pos, lst_head_tmp, tmp_lst_rel, tmp_lst_lvl)
...
def arrange_np_sent(lst_of_sent, lst_sent_np, lst_np_rel_sent, lst_np_pos_sent):
	print "\n inside arrange np nad np_rel \n"

	n = len(lst_of_sent)
	for i in range(n):
		print lst_sent_np[i]
		print lst_np_rel_sent[i]
		print lst_np_pos_sent[i]
		print
	...
	for i in range(n):
		tmp2 = []
		for j in lst_np_pos_sent[i]:
			# print "j", j
			idx = lst_of_sent[i].find(j[0])
			tmp2.append((idx, j))
	
		tmp3 = []
		for j in sorted(tmp2):
			tmp3.append(j[1])
		lst_np_pos_sent[i] = tmp3
	...
	return (lst_sent_np,lst_np_rel_sent,lst_np_pos_sent)
...
def corefer_plural_up( prp_idx, lst_of_sent, lst_sent_np, lst_np_pos_sent, lst_np_rel_sent, start_sent, level_up, last_name, prp_word, lst_ner_dict):

	print "\n inside corefer plural pronoun\n"

	print lst_np_pos_sent
				...
				if FLAG == 0:
					...
					if k < len(np_lst_up):
						...
						while k < len(np_lst_up):
							...
							# print "########",lst_sent_np[n]
							# print "@@@@@@@@", lst_np_rel_sent[n]
							# print "########", lst_np_pos_sent[n]

							# for relat in lst_np_rel_sent[n]:
							# 	print "sadDADS", relat

							rel = lst_np_rel_sent[n][k][1]
							print ">>>> rel >>>", lst_np_rel_sent[n][k][0], lst_np_rel_sent[n][k][1]
							print ">>>> pos >>>",p 
							if p[1] == "NNP" and w not in Location:
								print "rel ", rel, "w1  ", w
								if c == 0:
									if rel == "nsubj" or rel == "nn":
										print "if1",c,"rel ", rel, "w1  ", w
...
def pronoun_plural(lst_of_sent, lst_sent_np, lst_np_pos_sent, lst_np_rel_sent,lst_np_hw_sentnum_npnum, last_name, lst_ner_dict):
	print "\n inside plural sentences\n"
	n = len(lst_of_sent)
	# print lst_np_rel_sent
	(lst_sent_np,lst_np_rel_sent, lst_np_pos_sent) = arrange_np_sent(lst_of_sent, lst_sent_np, lst_np_rel_sent, lst_np_pos_sent)
	print "arranged\n"
	for i in range(n):
		print lst_sent_np[i]
		print lst_np_rel_sent[i]
	...
	for i in reversed(range(n)):
		...
		for idxx,j in enumerate(reversed(lst_sent_np[i])):
			if j.lower() in pluralPronouns:
					...
					if len(words) > 1:
						...
						for p in pos:
							...
							# if p[1] == "NNS":
								...
								idx2 = get_index(lst_np_hw_sentnum_npnum, j, i, id_dict[j])
								# print "idx"
								# print idx1
								# print idx2
					...
					if k < idx:
						...
						while k < idx:
					 	...
					 	if c > 1:
					 		...
					 		idx1 = get_index(lst_np_hw_sentnum_npnum, j, i, id_dict[j])
					 		print "tttt", j , idx1, id_dict[j]
				...
				if FLAG == 0:
					print "\n going upwards \n"
					corefer_plural_up(idx, lst_of_sent, lst_sent_np, lst_np_pos_sent, lst_np_rel_sent, i, 3, last_name, j, lst_ner_dict)
				...
				if n >= 0:
					...
					for k in reversed(range(len(np_lst_up))):
						...
						if pos == "NNP" and rel == "nsubj":
							...
							print "w1 ", w1
							FLAG = 1
							idx1 = get_index(lst_np_hw_sentnum_npnum, w, l, id_dict[w])
			...
			if w.lower() in singularPronounsNoMF:
				...
				for idx in range(i):
					...
					if pos == "NNP" and rel == "nsubj":
						...
						idx2 = get_index(lst_np_hw_sentnum_npnum, w1, l, id_dict[w1])
						print "idx"
						print idx1
						print idx2
						for ll in lst_np_hw_sentnum_npnum:
...
def pronoun_other(lst_of_sent, lst_sent_np, lst_ner_dict, lst_sent_pos):
	...
	for i in reversed(range(l)):
		...
		for j,k in enumerate(lst_words):
			# print "j  ", j
			flag = 0
			if k in otherPronouns:

				if k.lower() in id_dict:
					id_dict[k] += 1
				else:
					id_dict[k] = 0
				...
				right_window = 0
				for window in range (j+1, j+2):
					if lst_words[window].lower() == "are" or lst_words[window].lower() == "were":
						right_window = 1

				left_window = 0
				for window in range (j-4, j-1):	
					if lst_words[window].lower() == "are" or lst_words[window].lower() == "were":
						left_window = 1

				print "window left", left_window, " right window ", right_window
				temp = ''
				check = 0
				...
				if right_window == 0 and left_window == 0:
					if rnge != 9999999:

						for index in reversed(range(rnge+1)):
							print "id1=>>>>>", index, lst_sent_np[i][index]
							r = rel[index][1]
				...
				if flag == 0:
					if right_window == 1:
						print "still work left to do"
						print "Location ", Location
						print "rnge", rnge
						for idx in reversed(range(rnge+1)):
							np = lst_sent_np[i][idx]
							print "np ", np
							w = np.split()
							if len(w) > 1:
								pos = get_pos(np)
							else:
								pos = lst_sent_pos[i][idx]

							if len(w) > 1:
								for p in pos:
									if p[1] == "NNS" and np not in Location and ' '+k+' ' not in np: 
										temp += ' '+np
										flag = 1
										break
							else:
			...
			if w1.lower() in secondPersonPronouns:
				...
				for k in reversed(range(idx)):
					...
					if pos == "NNP":
						...
						idx1 = get_index(lst_np_hw_sentnum_npnum, w1, l, id_dict[w1.lower])
...
def get_final_result(lst_of_sent, lst_np_hw_sentnum_npnum):
	...
	for j,i in enumerate(lst_np_hw_sentnum_npnum):
		...
		idn = i[3]
		rw = " "+id_dict[idn]+" "
		if i[0].lower() in singularPronounsNoMF:
			if "\""+i[0].upper()+" " in lst_of_sent[sn]:
				w = "\""+i[0]+" "
			if "\""+i[0].lower()+" " in lst_of_sent[sn]:
				w = "\""+i[0]+" "
		# print sn ,w, rw
...
def final_process(lst_of_sent, lst_sent_np, lst_np_hw_sentnum_npnum, window):
	...
	while l >= 0:
		np_lst = ast.literal_eval(str(lst_sent_np[l]))
		for i in np_lst:
			c = np_lst.count(i)
			if c > 1:
				idx = np_lst.index(i)
...
def process_coreference(lst_of_sentences):
	...
	lst_np_hw_sentnum_npnum = pronoun_plural(lst_of_sent, lst_sent_np, lst_np_pos_sent, lst_np_rel_sent, lst_np_hw_sentnum_npnum, last_name, lst_ner_dict)
	print "\n cluster 3\n"
	for i,j in enumerate(lst_np_hw_sentnum_npnum):
		print i, j
	...
	lst_of_sent = pronoun_other(lst_of_sent, lst_sent_np, lst_ner_dict, lst_sent_pos)
...
if __name__ == "__main__":
	...
	last_name = open("/media/inno/01D0425114146710/Saurabh/Coreference/LastName/NameListV02.txt").read().split("\n")
	...
	process_coreference(lst_of_sentences)
