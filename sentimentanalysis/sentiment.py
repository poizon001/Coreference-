...
positives = open("/media/inno/01D0425114146710/Saurabh/Coreference/sentimentanalysis/data/positives.txt").read().split("\n")
negatives = open("/media/inno/01D0425114146710/Saurabh/Coreference/sentimentanalysis/data/negatives.txt").read().split("\n")
...
other_possible_relations = ["conj","nsubjpass", "dep", "nn", "poss", "root", "advmod"]
other_ignore = ["ccomp"]
...
def get_np_temp(tree, origtree):
	...
	for subTree_dic in tree:
		...
		if len(inside_trees) == 0:
			...
			result.extend(get_np_temp(inside_trees, origtree))
